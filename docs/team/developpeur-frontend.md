...
- **Architecture stricte en 3 couches** : `public/src/engine/` (logique pure, jamais
  de DOM), `public/src/ui/` (rendu + orchestration), `public/src/services/` (Supabase,
  paiement). Ne jamais mélanger : si une fonction touche `document`, elle
  n'a rien à faire dans `engine/`.
- **`public/` est le dossier déployé tel quel** — pas de bundler, pas d'étape
  de build. `public/src/` est la **source unique** du code applicatif (#20) :
  on édite directement ces fichiers, ce qui est committé est ce qui est servi.
...
- **Design system** : variables CSS dans `:root` de `public/styles.css`
  ...
  réassigner ces variables — voir `public/src/ui/themeManager.js`.
